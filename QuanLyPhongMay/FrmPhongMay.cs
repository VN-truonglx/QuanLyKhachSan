...
namespace QuanLyPhongMay
{
        ...
        
    {
        ...
        private void btnIn_Click(object sender, EventArgs e)
        {
            ...
            sql = "SELECT MaMay,TenMay,MaPM,MaOCung,MaDungLuong,MaChip,MaRAM,MaTocDo, MaManHinh,MaCoMH,MaChuot,MaBanPhim,MaODia,GhiChu where MaPM=N'" + cboMaPhongMay.Text + "'";
            tbl = QuanlyPhongMay.Class.Functions.GetDataToTable(sql);
            ...
            for (hang = 0; hang < tbl.Rows.Count; hang++)
            {
                //Điền số thứ tự vào cột 1 từ dòng 12
                exSheet.Cells[1][hang + 12] = hang + 1;
                for (cot = 0; cot < tbl.Columns.Count; cot++)
                //Điền thông tin hàng từ cột thứ 2, dòng 12
                {
                    exSheet.Cells[cot + 2][hang + 12] = tbl.Rows[hang][cot].ToString();
                    if (cot == 3) exSheet.Cells[cot + 2][hang + 12] = tbl.Rows[hang][cot].ToString();
                }
            }
